...
Theorem~\ref{theorem:distance_prof} to allow for arbitrary side-length
$s$ is trivial and omitted here}, $w=25.2$) we easily obtain the
expected distance-dependent connectivity profile for the created
sample graphs and, using this profile, generate purely
distance-dependent networks\footnote{label:
\smtcite{N1000-dist\char`_depend-flat\char`_graph-00-24.xml.gz}}. the
numerical will heavily rely on these distance-dependent networks to
identify aspects 
