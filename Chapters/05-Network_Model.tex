...
\begin{definition}[Anisotropic random graph model] Let $n \in
  \mathbb{N}$ and $w > 0$. The \textit{anisotropic random graph model}
  $G(n,w)$ is a probability space over the set of anisotropic
  geometric graphs with a  probability distribution induced by the
  following process: Let $G$ be an empty graph with $n$
...
\begin{theorem} \label{distance_prof} Let $(G,\Phi, a)$ represent an
  ...
  that is such that $C(x)$ is the probability that a vertex pair $v,w
  \in V(G)\setminus\Delta_{V(G)}$ in distance $x = \norm{\Phi(v)-\Phi(w)}$ is
  connected. Then
  \[
    C(x) = \begin{cases}%
...
\begin{proof}
  Let $v,w$ be a pair of vertices in $V(G) \setminus \Delta_{V(G)}$ in distance
  $\norm{\Phi(v)-\Phi(w)} = x$ of each other. 
