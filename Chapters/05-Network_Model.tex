...
\begin{definition}[Anisotropic geometric graph]
  ...
  tuple $(G,\Phi,a)$, of a simple directed graph $G$ with $|V(G)|=n$ vertices and
  the maps \[\Phi:V(G)\to[0,1]^2, \quad a:V(G)\to[0,2\pi),\] such that for every
  vertex pair $v,v' \in V(G)$ and edge $e\in E(G)$ with $s(e)=v$ and
  $t(e)=v'$ exists if and only if $\le w$. where denotes euclidean
  distance l2 norm %what is \le w?? Norm ??
\end{definition}
...
\begin{remark}
  ...
  surface to be the unit square. This does not limit the model, as in
  fact only the relative width of the axon band in regards to the
  surface's side length is determining connectivity statistics;
  % ------------------------------------------------ 
  \marginpar{anisotropic model is scale-free}
  % ------------------------------------------------
  the expected number of connections is easily obtained by division of
  the area covered by the axon and the area of the square, making
  connectivity statistics int the anisotropic random graph model
  scale-free. 
\end{remark}
