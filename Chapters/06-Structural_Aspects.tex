...
%
% The out-degree in
% anisotropic networks can thus be approximated by the distribution of
% axon lengths in the network.
%
...
%   If $f(l)$
% the probability density function to have axon length $l$ for a random
% node with random orientation, the out-degree distribution can be
% approximated by

% With parameters $N = 1000$ and $\frac{w}{s} = 0.252$, we obtain an
% upper bound for the expected out-degree using $\frac{l}{s} \leq \sqrt{2}$,
% \[
% \frac{l}{s} \leq
% \sqrt{2}
% \]
% reflecting the maximum out-degree extracted from the simulated
...
\begin{figure}[H]
  ...
  \caption{\textbf{Characteristic out-degree distribution as an
    ...
    size. (cf. \autoref{suppfig:out_degree}) (\smtcite{019555b0})}
...
% Even in the state of the art
% experimental setup of \textcite{Perin2011}, they note that 
...
% (\autoref{fig:in_degree_ER_compare}).
