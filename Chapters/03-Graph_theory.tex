...
\textit{binomial random graph}. %\parencite{Janson_Random-graphs}
As typical neuronal networks are large ($n \geq 10^3$) with
sparse connectivity ($p \approx  0.1$), in- and out-degree distribution
can be approximated by a Poisson distribution, $P^X(k) \approx
...
\begin{lemma} Let $D$ be a random variable mapping to the distance of
  two random points in the unit square. Then
\[P(X=x) = \]
\end{lemma}
%
\begin{proof}
The argument after \textcite{Philip2007}
\end{proof}

\begin{proposition}
overall cp 
\[
  p = \int_0^{\sqrt{2}} C(x) P^D(x) \, dx
\]
\end{proposition}
%
\begin{proof}
