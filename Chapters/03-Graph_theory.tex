...
%   \includegraphics[width=0.49\textwidth]{tikz/directed_pseudograph.pdf}
%   \hfill
%   \includegraphics[width=0.49\textwidth]{tikz/directed_multigraph.pdf}
...
\begin{definition}[Gilbert random graph model]\label{def:gilbert_random_graph}
Let $n\in\mathbb{N}$ and $0\leq p \leq 1$. The \textbf{Gilbert random graph
model} $G(n,p)$ is a probability distribution over $G^n$ such that any
graph $G \in G^n$ with $\abs{E(G)}=m$ occurs with probability
$p^m(1-p)^{m-n(n-1)}$, for $0 \leq m \leq n(n-1)$. 

 $G(n,p)$ is a graph $G$ with
$\abs{V(G)}=n$ obtained by randomly adding edges between every vertex
pair with a probability $p$.
\end{definition}

Alternatively, the Gilbert random graph model can be defined as a
stochastic process. 
...
\begin{definition}[Geometric graph] A geometric directed graph is a
  directed graph $G$ paired with a map
\[
  p:V(G) \to [0,1)^2,
\]
such that for every vertex pair $v,w \in V(G)$ an edge $e \in E(G)$
with $s(e)=v$ and $t(e)=w$
\end{definition}

This notion diverges from the usual . In directed graphs. We are
however using . Random geometric graphs: \textcite{Gilbert1961}
