...
Denote the set of geometric graphs with $n$ edges by $G_{\Phi}^n$.
...
\begin{remark}
  We call $C$ the graph's \textit{distance-dependent connection
    ...
    probability}. Clearly, connectivity statistics in the
...
\begin{lemma} Let $X,Y$ be independent random variables, denote with
  $f_X$ and $f_Y$ their probability distribution functions.
  \begin{enumerate}[label=(\arabic*),ref=(\arabic*)]
    \item The distribution of $f_{X+Y}$
    \item The distribution of $f_{X^2}$
    \item Let $X$ be positive. The distribution of $f_{\sqrt{X}}$
  \end{enumerate}
\end{lemma}
...
%
\begin{proof}
  This result is well known, here we follow the approach described by
  \textcite{Moltchanov2012}.  Consider two independently and uniformly
  distributed random points $p_1 = (x_1,y_1)$ and $p_2 = (x_2,y_2)$ in
  $[0,s]^2$. The distance between $p_1$ and $p_2$ is given as
  \[
  ...
  independent random variables.  % Klenke 14.19
  ...
  As a next step we calculated the distribution for $\Delta_x^2 =
  (x_1-x_2)^2$. The probability density function of thus for . Thus 
...
\marginpar{\vspace{1.8cm}\\ label:
  ...
  in Appendix~\ref{sec:reproducibility}} %?? correct ref
%------------------------------------------------
