...
\begin{definition}[Directed graphs]\index{directed graph}
...
% Let $n\in\mathbb{N}$ and $0\leq p \leq 1$. The \textit{Gilbert random graph
% model} $G(n,p)$ is a probability distribution over $G^n$ such that any
% graph $G \in G^n$ with $\abs{E(G)}=k$ occurs with probability
% $p^k(1-p)^{k-n(n-1)}$, for $0 \leq k \leq n(n-1)$. 
% \end{definition}
