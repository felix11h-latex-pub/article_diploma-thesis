...
The Gilbert random graph is therefore also often referred to the
\textit{binomial random graph model}.  As the typical neuronal networks
are large ($n of order 10^3$) with sparse connectivity ($p \sim 0.1$), in-
and out-degree distribution can be approximated by a Poisson
distribution, 
\[
P^X(k) \approx \operatorname{Pois}_{\lambda}(k), \,\,
\mathrm{with}\,\, \lambda = (n-1)p,
\]
after the Poisson limit theorem. Most results in the study of random
graph models consider $n\to \infty$. In this study we are more concerned
...
\begin{remark} This definition diverges from the usual notion of a
  ...
  (usually $l_w$) \parencite[108]{Mesbahi_Multiagent-networks}
  Moreover, geometric graphs are usually only discussed in the context
  of random graph models \parencite{Gilbert1961}.  We denote the set
  of geometric graphs with $n$ edges by $G_{\Phi}^n$.  While the
  concept of spatially embedded graph allows to define a random graph
  model that is highly relevant for this study:
\end{remark}
...
\begin{lemma} Distance occurences of two random points.
\end{lemma}
%
\begin{proof}
Heere
