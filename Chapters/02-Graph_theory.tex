...
\begin{comment}
  ...
  functions $s',t'$ do not need to be specified, since they are
  uniquely determined as the projections on the first and second
  component, $s' = \mathrm{pr}_1, t' = \mathrm{pr}_2$. Wait, not so
  easy because multi-sets, however add synaptic strength of parallel
  edges to make directed loop-graphs! (Bang-Jensen)
\end{comment}
...
\begin{comment}
  \begin{defn}[Neighbour, adjacent]
    Two \textbf{vertices} $x,y \in V(G)$ of $G$ are called
    \textit{adjacent} or \textit{neighbours} if there is an edge
    between $x$ and $y$, $(x,y) \in E(G)$. Two \textbf{edges} $e \neq
    f$ are \textit{adjacent} if they have an end in common.
  \end{defn}
\end{comment}
...
Wikipedia,\textbf{ West-GraphTheory}!!
