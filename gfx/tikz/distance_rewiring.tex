...
\begin{tikzpicture}[inmargin_node/.style={draw, very thick, circle, inner
  ...
  \draw[ultra thick, use as bounding box] (0,0) rectangle (\Edge,\Edge);

  \node[outmargin_node]  (Origin) at (4,4) {};
  ...
  \node[inmargin_node] at ($(Origin)+(227:\Distance+\Margin*-0.2)$) {};
  ...
  \draw[very thick, ->] (Origin) -- (t2);

  \node[label=above:{\huge2$\omega$}] (width_center) at ($(Origin)+(0:\Distance)$) {}; 
